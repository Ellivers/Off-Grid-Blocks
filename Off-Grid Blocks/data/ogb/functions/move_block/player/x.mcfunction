...
# Tells all your blocks to move in teh positive X direction
scoreboard players operation #temp_ID off_grid_blocks = @s ogb.select_id
execute as @e[type=#ogb:blocks,tag=ogb.block,tag=off_grid_blocks] if score @s ogb.select_id = #temp_ID off_grid_blocks run function ogb:move_block/block/x
